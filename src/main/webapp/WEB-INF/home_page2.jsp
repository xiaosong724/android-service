...
<header class="htmleaf-header">
    ...
    <div style="width: 100%">
        <nav id="gooey-h">
            ...
            <a href="#" class="gooey-menu-item" data-toggle="modal" data-target="#exampleModal_up"> <i
                    class="fa fa-bicycle">改</i> </a>
            ...
            <a href="#" class="gooey-menu-item" data-toggle="modal" data-target="#exampleModal_back"><i
                    class="fa fa-rocket">添加</i> </a>
    ...
    <nav aria-label="...">
        <ul class="pager">
            <li class="previous"><a href="/home?page=${loveLogs.prePage}" style="color: black"><span aria-hidden="true">&larr;</span>
                Older</a></li>
            <li class="next"><a href="/home?page=${loveLogs.nextPage}" style="color: black">Newer <span
...
<script>
        ...
        setTimeout(function () {
                $.ajax({
                    ...
                    success: function (msg) {
                        ...
                        for (var imgp in msg) {
                            var leixing="";
                            if(msg[imgp].filetype=="img"){
                                $("#imgshow_log").html($("#imgshow_log").html() + "\n" +
                                    "<p>图片ID:"+ msg[imgp].imgid +"</p><img src=\"/img/mini/" + msg[imgp].imgpath + "\" id=\"" + msg[imgp].imgid + "\" onclick=\"show_hide_title('" + msg[imgp].imgpath + "')\" class=\"songsize\">")
                            }else {
                                $("#imgshow_log").html($("#imgshow_log").html() + "\n" +
                                    "<p>视频ID:"+ msg[imgp].imgid +"</p><video poster='' src=\"/img/" + msg[imgp].imgpath + "\" id=\"" + msg[imgp].imgid + "\"  data-ckin=\"default\"class=\"songsize\" click='yes' onclick='playvideo(this)'></video>")
                            }
    ...
    //刷新局部,重新加载动态添加元素的样式和点击事件
    function playvideo(my){
        if($(my).attr("click")=="yes"){
            $(my).load("http://localhost:8080/home");
            $(my).attr("click","no");
            $(my).attr("class","");
        }
    }
   ...
   /*---------------------------------------修改日志------------------------------------------*/
    ...
    function val_up(loveLog) {
        ...
        for (var imgpa in imgp) {
            if(imgp[imgpa].filetype=="img"){
                img_val.html(img_val.html() +
                    "<div><button style='display: none;float: left;position: absolute;' ltitle=\"" + loveLog[0].title + "\" lid=\"" + loveLog[0].id + "\" dename=\"" + imgp[imgpa].imgpath + "\"deid=\"" + imgp[imgpa].imgid + "\" onclick=\"delete_img(this)\" type=\"button\"class='btn btn-danger'>删除</button><img onclick='show_button(this)' id=\"" + imgp[imgpa].imgid + "\" class=\"songsize\" src=\"img/mini/" + imgp[imgpa].imgpath + "\"></div>"
                );
            }else {
                img_val.html(img_val.html() +
                    "<div><p>视频</p><button style='display: none;float: left;position: absolute;z-index:9999;' ltitle=\"" + loveLog[0].title + "\" lid=\"" + loveLog[0].id + "\" dename=\"" + imgp[imgpa].imgpath + "\"deid=\"" + imgp[imgpa].imgid + "\" onclick=\"delete_img(this)\" type=\"button\"class='btn btn-danger'>删除</button><video onclick='show_button(this)' id=\"" + imgp[imgpa].imgid + "\" class=\"songsize\" src=\"img/" + imgp[imgpa].imgpath + "\"></div>"
                );
            }
    ...
    function delete_img(my) {
        ...
        var rootname=$.cookie("user_name_log");
        ...
        $.ajax({
            url: "/deleteimgid",
            type: "post",
            data: "imgid=" + imgid + "&impath=" + impath + "&logid=" + logid + "&title=" + ltitle+"&rootname="+rootname,
            dataType: "json",
            success: function (msg) {
    ...
    $("#scscsc").on('change', function (e) {
        ...
        //image_holder.empty();
        if (extn == "gif" || extn == "png" || extn == "jpg" || extn == "jpeg" || extn == "mp4" || extn == "avi" || extn == "rmvb" || extn == "rm") {
            if (typeof (FileReader) != "undefined") {
                ...
                for (var i = 0; i < countFiles; i++) {
                    ...
                    reader.onload = function (e) {
                        ...
                        console.log((e.target.result).indexOf("data:image/"))
    ...
    function update_log_text() {
        ...
        formDatalog.append("rootname", rootname);
...
/*===================加载等待=====================*/
function jiazai_await(s){
    if(s==0){
        $(".pacman").show();
        $(".zeromodal-overlay").show();
    }else {
        $(".pacman").hide();
