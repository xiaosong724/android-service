...
import com.fangou.fangouapp.vo.JpushConfig;
...
public class JpushService {
    ...
    public PushResult sendPush(String title, String content, Map<String, String> extrasMap, String... alias) {
        ...
        try {
            ...
            System.out.println("极光推送结果 - " + result);
        } catch (APIConnectionException e) {
            LOG.error("极光推送连接错误，请稍后重试 ", e);
            LOG.error("Sendno: " + payload.getSendno());
            ...
            System.out.println("以下存在不能识别别名: " + alias);
            LOG.error("Sendno: " + payload.getSendno());
        }
        return result;
    }
     ...
     * 
     * @see使用NettyHttpClient,异步接口发送请求通过回调函数可以获取推送成功与否情况
     */
